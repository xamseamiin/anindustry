  ...
  const scanReceipt = async (file: File) => {
    ...
    try {
      ...
      if (scan.items?.length) {
        const explicitLineNames = scan.items.map((item: any) => String(item.customerName || '').trim()).filter(Boolean);
        const mixedCustomerReceipt = new Set(explicitLineNames.map(normalizeLookup)).size > 1;
        if (mixedCustomerReceipt) {
          // The header selector is a one-customer shortcut. It must never leak the
          // first recognized buyer into another buyer's row on a mixed receipt.
          setCustomerId('');
          receiptCustomerId = '';
          setRecognizedCustomerName('');
          setCustomerCorrectionConfirmed(false);
        }
        setItems(scan.items.map((item: any) => {
          ...
          const lineCustomerName = String(item.customerName || (!mixedCustomerReceipt ? (receiptCustomerName || globalRawCustomerName) : '')).trim();
    ...
    <section className="rounded-3xl border border-cyan-400/30 bg-slate-900/70 p-4 space-y-3"><div className="flex items-center justify-between"><h2 className="text-lg font-black">New Sale</h2><span className="text-xs font-black text-cyan-300">{total.toLocaleString()} ETB</span></div>
      ...
      <div className="space-y-2">
        ...
        {items.map((item, index) => <div key={`${item.productId}-${index}`} className="space-y-2 rounded-xl border border-white/10 bg-white/5 p-2">
          ...
          <label className="block text-[9px] font-black uppercase text-slate-500">Customer-ka item-kan<select value={item.customerId || ''} onChange={e => setItems(current => current.map((x, i) => i === index ? { ...x, customerId: e.target.value, customerName: null, customerCorrectionConfirmed: Boolean(x.ocrCustomerName) } : x))} className="mt-1 w-full rounded-lg border border-white/10 bg-slate-950 p-2 text-[11px] normal-case text-white"><option value="">Default customer / Walk-in</option>{customers.map(c => <option key={c.id} value={c.id}>{c.name}{c.phone ? ` · ${c.phone}` : ''}</option>)}</select></label>
        </div>)}
      </div>
