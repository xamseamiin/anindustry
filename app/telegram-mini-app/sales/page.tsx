...
type Item = { productId: string; productName: string; quantity: number; unitPrice: number };
...
export default function TelegramSalesPage() {
  ...
  const addProduct = (productId: string) => {
    ...
    if (!product) return;
    setItems(current => { const existing = current.find(item => item.productId === product.id); if (existing) return current.map(item => item.productId === product.id ? { ...item, quantity: Math.min(item.quantity + 1, product.inStock) } : item); return [...current, { productId: product.id, productName: product.name, quantity: 1, unitPrice: product.sellingPrice || 0 }]; });
  };
  ...
  const saveSale = async () => {
    ...
    try { const response = await fetch('/api/telegram/sales', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ customerId: customerId || null, accountId: accountId || null, paymentMethod, paidAmount: paymentAllocations.length ? selectedPaymentTotal : Number(paidAmount || total), paymentAllocations, items, receiptUrl, receiptHash }) }); const data = await readJson(response); if (!response.ok) throw new Error(data.error); setMessage('Sale waa la kaydiyey: ' + data.sale.invoiceNumber); setItems([]); setPaidAmount(''); setPaymentAllocations([]); await loadSalesData(); } catch (error: any) { setMessage(error.message || 'Sale lama kaydin.'); } finally { setSaving(false); }
  };
  ...
  return <main className="min-h-screen bg-[#020617] text-slate-100 px-4 py-4 pb-28 font-sans"><div className="mx-auto max-w-md space-y-4">
    ...
    <section className="rounded-3xl border border-cyan-400/30 bg-slate-900/70 p-4 space-y-3"><div className="flex items-center justify-between"><h1 className="text-lg font-black">New Sale</h1><span className="text-xs font-black text-cyan-300">{total.toLocaleString()} ETB</span></div>
      ...
      {items.map((item, index) => <div key={`${item.productId}-${index}`} className="grid grid-cols-[1fr_55px_78px_28px] items-center gap-2 rounded-xl bg-white/5 p-2"><span className="truncate text-xs font-bold">{item.productName}</span><input type="number" min="1" value={item.quantity} onChange={e => setItems(current => current.map((x, i) => i === index ? { ...x, quantity: Number(e.target.value) } : x))} className="rounded-lg bg-slate-950 p-2 text-xs" /><input type="number" min="0" value={item.unitPrice} onChange={e => setItems(current => current.map((x, i) => i === index ? { ...x, unitPrice: Number(e.target.value) } : x))} className="rounded-lg bg-slate-950 p-2 text-xs" /><button onClick={() => setItems(current => current.filter((_, i) => i !== index))} className="text-rose-300"><Trash2 size={15} /></button></div>)}
