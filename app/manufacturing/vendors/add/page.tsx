...
import {
  ...
  Package, FileText, CheckCircle2, AlertCircle
} from 'lucide-react';
...
export default function AddVendorPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setTimeout(() => router.push('/manufacturing/vendors'), 1500);
      } else {
        const data = await res.json();
        setToast({ message: data.error || 'Failed to create vendor', type: 'error' });
      }
    } catch (e) {
  ...
  const inputClass = "w-full p-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold text-slate-700 outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500/40 transition-all";

  return (
    <div className="flex flex-col gap-8 p-4 lg:p-8 max-w-5xl mx-auto min-h-screen pb-20">
      {/* Header */}
      <div className="flex items-center gap-6">
        <Link href="/manufacturing/vendors" className="p-3 bg-white rounded-2xl shadow-sm border border-slate-200 text-slate-500 hover:text-blue-600 transition-all hover:scale-105 active:scale-95">
          <ArrowLeft size={24} />
        </Link>
        <div>
          <h1 className="text-4xl font-black text-slate-900 tracking-tight">Add Partner</h1>
          <p className="text-slate-500 font-medium">Onboard a new supplier or service provider.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white/60 backdrop-blur-xl p-8 rounded-[2.5rem] border border-white shadow-2xl space-y-8">
            <div className="flex items-center gap-3 pb-4 border-b border-slate-100">
              ...
              <h3 className="text-xl font-black text-slate-900">Company Details</h3>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Vendor / Company Name *</label>
                <input
                  type="text"
                  placeholder="e.g. Som-Steel Solutions"
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Partner Category</label>
                <select
                  className={inputClass}
                  value={formData.type}
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Products & Services</label>
              <textarea
                placeholder="Briefly describe what they provide..."
        ...
        <div className="space-y-6">
          <div className="bg-white/60 backdrop-blur-xl p-8 rounded-[2.5rem] border border-white shadow-2xl space-y-6">
            ...
            <textarea
              placeholder="Physical address / Warehouse location..."
          ...
          <div className="bg-slate-900 p-8 rounded-[2.5rem] shadow-2xl text-white space-y-6">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-4 bg-blue-600 hover:bg-blue-500 disabled:bg-slate-700 text-white rounded-2xl font-black text-sm uppercase tracking-[0.2em] shadow-xl shadow-blue-600/20 transition-all flex items-center justify-center gap-3"
            >
